...
describe('E2E: GET /api/v1/policies', () => {
    it('should return 401 error without auth header', () => {
        return server.get('/api/v1/policies')
            .then(response => {
    ...
    it('should return correct number of policies with limit query', () => {
        return login.then(loginResponse => {
            server.get('/api/v1/policies?limit=5')
                .set('Authorization', `Bearer ${loginResponse.body.token}`)
                .then(response => {
...
describe('E2E: GET /api/v1/policies/:id', () => {
    it('should return 401 error without auth header', () => {
        return server.get('/api/v1/policies/dummy')
            .then(response => {
                assert.strictEqual(response.statusCode, 401)
            })
    })
        ...
        return login.then(loginResponse => {
            server.get('/api/v1/policies')
                ...
                .then(originalResponse => {
                    server.get(`/api/v1/policies/${originalResponse.body[0].id}`)
                        .set('Authorization', `Bearer ${loginResponse.body.token}`)
                        .then(response => {
                            assert.strictEqual(response.statusCode, 200)
                        })
    ...
    it('should return 404 response if id not found', () => {
        return login.then(loginResponse => {
            server.get(`/api/v1/policies/dummy`)
                .set('Authorization', `Bearer ${loginResponse.body.token}`)
                .then(response => {
